...
export default function ProductPage({ params }) {
    ...
    useEffect(() => {
        // Ensure params is available and has the ID
        if (unwrappedParams?.id) {
            setProdID(unwrappedParams.id);  // Set the product ID
        } else {
    ...
    // Fetch the product details based on `prodID`
    useEffect(() => {
        if (prodID) {
            const fetchProduct = async () => {
                const docRef = doc(db, "Products", prodID); // Use `prodID` directly
    ...
    }, [prodID]);  // Run this effect when `prodID` changes

    // Fetch the user data on auth state change
    useEffect(() => {
                    ...
                    <div className="flex flex-col lg:flex-row relative w-full">
                        ...
                        <div className="lg:w-1/2 lg:pl-6 mt-6 lg:mt-0 flex flex-col justify-center">
                            ...
                            <form action="" className="space-x-3 flex items-center mt-4">
                                ...
                                <p className="text-xl font-bold underline underline-offset-4 bg-orange-500 rounded-full p-2 text-white ">{`${product.Price} AED`}</p>
                                <div>
